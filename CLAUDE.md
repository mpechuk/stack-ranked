...
- **Feedback deck** — 18 cards (9 Positive +2 / 9 Constructive −2). `resolveFeedbackPhase(state, hooks)` runs at the **top of every Review, before `runReview`** (called from `postmortem`, async so humans can be asked). Deal 1/player, keep-or-give, net folds into Review Score + CEO tiebreak, clamped to ±`feedbackNetCap` (=4). Dials: `feedbackValue`(2), `feedbackNetCap`(4), `feedbackTarget`(`'score'` default | `'rung'` | `'blend'` | `'spread'`), `feedbackNegLeaderOnly`, `feedbackBlendPcWeight`(6). Transient — never touches persistent PC. UI: `hooks.decide({action:'giveFeedback', options:[…]})` returns a playerId; review summary rows carry `feedback`.
...
Public: `SR.actions.{shareProject,contribute}`, `SR.helpers.sharedProjects`. Internal (tests): `SR._internal.{resolveFeedbackPhase,contributeToProject,completeCollaborative,sharedProjects,threatLeader,buildRules}`.

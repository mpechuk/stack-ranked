...
- **Feedback deck** — 18 cards (9 Positive +2 / 9 Constructive −2). `resolveFeedbackPhase(state, hooks)` runs at the **top of every Review, before `runReview`** (called from `postmortem`, async so humans can be asked); it's a **dispatcher** on `feedbackMode` → `resolveFeedbackClassic` or `resolveFeedbackGiveOne`, then a shared `tallyFeedback`. Net folds into Review Score + CEO tiebreak, clamped to ±`feedbackNetCap` (=4). Dials: `feedbackMode`(`'classic'` default | `'give-one'`), `feedbackValue`(2), `feedbackNetCap`(4), `feedbackTarget`(`'score'` default | `'level'` | `'blend'` | `'spread'`), `feedbackNegLeaderOnly`, `feedbackBlendPcWeight`(6). Transient — never touches persistent PC. UI is selectable ("Feedback Round" on setup → `config.rules.feedbackMode`).
...
4. **Changed the CAREER LADDER** — `leaderboard.md` holds the levels / AP / CC thresholds / badges (mirror in `game.js` constants + spec §6.1) **and** the **"Stack Rank Formula" = the Review Score formula**. So changes to *either* the ladder numbers *or* review scoring must edit `leaderboard.md` and **regenerate the career-ladder PDF**.

...
- **Transport:** PeerJS (`window.Peer`, a **CDN global** in `index.html`) uses its **free public broker** (`0.peerjs.com`) for **signalling only**; data flows P2P once the `DataConnection` opens. `net.js` exposes `hostRoom`/`joinRoom` (both `Promise<api>`), using **`serialization:'none'`** with our own framing: control msgs are JSON strings, big `STATE_UPDATE`s are gzip'd `ArrayBuffer`s (built-in `CompressionStream`, gated by the client's `caps.z`; peers without it get plain JSON). PeerJS `network`/`server-error`/`socket-error`/`disconnected` are **transient** → `peer.reconnect()`, room stays up.

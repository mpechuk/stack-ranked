...
- **Deploy gotcha:** the GitHub Pages workflows (`.github/workflows/deploy-pages.yml` and `pr-preview.yml`) copy a **hardcoded file list** into `_site` (`cp index.html game.js net.js _site/`). **Any new runtime file the page loads must be added to both `cp` lines** or it 404s on the deployed/preview site (symptom: `window.SRNet` undefined → "online play unavailable" even though WebRTC works locally).

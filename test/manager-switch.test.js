 ...
 * arms (transfers OFF vs ON) from synergy_corp_manager_switch_test.js and asserts
 * the causal verdict — being able to switch away from a punishing early boss must
 * materially lift the stuck cohort's win share and final level.
 *
 * Full human-facing printout:
 *   node synergy_corp_manager_switch_test.js 1000   (or: npm run sim:transfer)
 */
'use strict';
const { test } = require('node:test');
const assert = require('node:assert');
const ms = require('../synergy_corp_manager_switch_test.js');
...
test('Request-a-Transfer lets a badly-managed cohort catch up', { timeout: 90000 }, async () => {
  ...
  assert.ok(v.levelLift > 0.3,
    `bad-cohort avg final level barely improved (lift ${v.levelLift.toFixed(2)})`);
  assert.ok(v.causal, 'causal catch-up verdict failed — inspect `node synergy_corp_manager_switch_test.js`');
});
